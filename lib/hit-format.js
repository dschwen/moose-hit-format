...
import hitFormatExecutables from 'hit-format-binaries';
...
export default class ClangFormat {
  ...
  format = (editor) => {
    ...
    let exe = atom.config.get('hit-format.executable');
    if (!exe) {
      const exePackageLocation = path.dirname(hitFormatExecutables.location);
      if (os.platform() === 'win32') {
        exe = path.join(exePackageLocation, '/bin/win32/hit-format.exe');
      } else {
        exe = path.join(exePackageLocation, `/bin/${os.platform()}_${os.arch()}/hit-format`);
      }
    }

    const options = {
      style: atom.config.get('hit-format.style'),
      cursor: this.getCurrentCursorPosition(editor).toString(),
      'fallback-style': atom.config.get('hit-format.fallbackStyle'),
    };
    ...
    // Pass file path to hit-format so it can look for .hit-format files
    ...
    try {
      ...
      if (error.message.indexOf('Command failed:') < 0) {
        ...
        atom.notifications.addError('Clang Format Command Failed', {
          dismissable: true,
          detail: `hit-format failed with the below error. Consider reporting this by creating an issue here: https://github.com/LiquidHelium/atom-hit-format/issues.\nError message: "${error.stderr.toString()}".\nWhen running: "${exe} ${args}".\nStdout was: "${error.stdout.toString()}"`,
        });
      }
    }
  }
  ...
  shouldFormatOnSaveForScope = (scope) => {
    if (atom.config.get('hit-format.formatCPlusPlusOnSave') && ['source.c++', 'source.cpp'].includes(scope)) {
      return true;
    }
    if (atom.config.get('hit-format.formatCOnSave') && ['source.c'].includes(scope)) {
      return true;
    }
    if (atom.config.get('hit-format.formatObjectiveCOnSave') && ['source.objc', 'source.objcpp'].includes(scope)) {
      return true;
    }
    if (atom.config.get('hit-format.formatJavascriptOnSave') && ['source.js'].includes(scope)) {
      return true;
    }
    if (atom.config.get('hit-format.formatTypescriptOnSave') && ['source.ts'].includes(scope)) {
      return true;
    }
    if (atom.config.get('hit-format.formatJavaOnSave') && ['source.java'].includes(scope)) {
      return true;
    }
    return false;
  }
  ...
  // Returns line numbers recognizable by hit-format, i.e. '<begin>:<end>'.
