...
export default class HitFormat {
  ...
  handleBufferEvents = (editor) => {
    ...
    const bufferSavedSubscription = buffer.onWillSave(() => {
      ...
      if (atom.config.get('hit-format.formatOnSave') &&
  ...
  format = (editor) => {
    ...
    if (!exe) {
      if (os.platform() === 'win32') {
        ...
        exe = path.join(__dirname, `hit.${os.platform()}.${os.arch()}`);
      }
    }
    ...
    const style = atom.config.get('hit-format.style') || path.join(__dirname, 'default.style');
